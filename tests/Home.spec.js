...
test('TC007 - Verify Top 10 Movies Section and Video Playback (Dynamic)', async ({ page, request }) => {
  ...
  await expect(page.getByRole('img', { name: movieTitle }).first()).toBeVisible();
  ...
  if (hasTrailer) {
    ...
    if (iconCount > 0) {
      ...
      // Close trailer modal
      let closed = false;
      for (const selector of ['[role="dialog"] svg', '[aria-label="Close"]', 'button:has(svg)']) {
        try {
      ...
      console.warn(`⚠ Trailer icon missing for movie with trailer: ${movieTitle}`);
    }
  } else {
  ...
  const top10Container = page.locator('div').filter({ hasText: 'Top 10 Movies' }).first();

  const bookNowBtn = top10Container.getByRole('link', { name: 'Book Now' }).first();
...
test('TC009 – Verify First Two Offers Display Correctly in English and Arabic Using Offers API', async ({ page, request }) => {
  ...
  const orderedOffers = offers.sort((a, b) => a.group_order - b.group_order).slice(0, 2);

  console.log(`🎯 Testing first 2 offers (by group_order):`);
  orderedOffers.forEach((offer, idx) => {
    console.log(`   ${idx + 1}. ${offer.name} (ID: ${offer.id}, Order: ${offer.group_order})`);
  });
...
test('TC011 - Verify Homepage Footer Links and Social Media Integration', async ({ page }) => {
  ...
  await expect(
    page.getByRole('contentinfo').getByRole('link', { name: 'Need Assistance ?' })
  ).toBeVisible();
  ...
  await expect(
    page.locator('div').filter({ hasText: /^Connect with Novo$/ }).first()
  ).toBeVisible();
