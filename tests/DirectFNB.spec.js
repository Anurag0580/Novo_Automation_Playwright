...
test("TC_01 – Verify Direct F&B Cinema Listing Using Cinema Details and Timings API Validation", async ({ page, request }) => {
  ...
  await expect(page.getByRole('button', { name: 'Log in to see your upcoming' })).toBeVisible();
  await page.getByRole('button', { name: 'Log in to see your upcoming' }).click();
  await loginAndCaptureTokenDirectFNB(page);
  
  await expect(page.getByRole('button', { name: 'CLICK HERE to order F&B' })).toBeVisible();
  ...
  const cinemaDetails = await fetchCinemaDetails(request, 19.1517288, 72.8341961);
  ...
  console.log(`✅ Cinema Details API: ${cinemaDetails.data.length} cinemas found`);
  console.log(`✅ Cinema Timings API: ${cinemaTimings.data.length} timing entries found`);
  ...
  for (const [cinemaId, timing] of todayTimingsMap.entries()) {
    ...
    try {
      const cinemaLocator = page.locator('div').filter({ 
        hasText: new RegExp(cinemaDetail.name, 'i') 
      }).filter({ 
        hasText: /Open:\s*\d{1,2}:\d{2}(?:am|pm)\s*-\s*\d{1,2}:\d{2}(?:am|pm)/ 
      });
      ...
      if (count > 0) {
        ...
        const timeMatch = text.match(/Open:\s*(\d{1,2}:\d{2}(?:am|pm))\s*-\s*(\d{1,2}:\d{2}(?:am|pm))/);
        
        if (timeMatch) {
          const displayedOpen = timeMatch[1];
          const displayedClose = timeMatch[2];
          const timesMatch = displayedOpen === timing.open && displayedClose === timing.close;
  ...
  console.log(`Match rate: ${((matchedCinemas / todayTimingsMap.size) * 100).toFixed(1)}%`);
  console.log('═══════════════════════════════════════════════════\n');
  
  expect(foundCinemas).toBeGreaterThan(0);
  expect(matchedCinemas).toBeGreaterThan(0);
  
  await page.screenshot({ path: 'takeaway-cinema-validation.png', fullPage: true });
  ...
  const selectedCinemaLocator = page.locator('div.relative.cursor-pointer.flex')
    .filter({ hasText: selectedCinema.name })
    .filter({ hasText: new RegExp(`Open:\\s*${selectedCinema.displayedOpen}\\s*-\\s*${selectedCinema.displayedClose}`) });
  ...
  console.log('\n📊 F&B API Data Loaded');
  console.log(`   Categories available: ${concessionsData.data?.length || 0}`);
});

test('TC_02 – Verify Direct F&B Order with Item Without Modifiers and Successful Checkout', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
  ...
  const selectedCinemaName = cinemaDetails.data.find(c => c.id === firstActiveCinemaId)?.name || '';
  const cinemaLocator = page.locator('div.relative.cursor-pointer.flex')
    .filter({ hasText: new RegExp(selectedCinemaName, 'i') });
  
  // USE THE NEW COMBINED FUNCTION
  const concessionsData = await clickCinemaAndNavigateToFandB(page, cinemaLocator.first(), firstActiveCinemaId);
...
await Promise.all([
  page.waitForURL(
    url => url.pathname.includes('/takeaway/confirm'),
    { timeout: 30000 }
  ),
...
test('TC_03 – Verify Direct F&B Order with Item Having Modifiers and Successful Checkout', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
...
test('TC_04 – Verify Direct F&B Order with Item Having Alternates and Successful Checkout', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
...
test('TC_05 – Verify Direct F&B Order with Item Without Modifiers Using Gift Card Payment', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
...
await completePaymentWithGiftCard(page, request,authToken, fbTracker.totalPrice);
...
// --- F&B row (+ QAR XX) ---
await expect(
  page.getByText('F&B').nth(1)
).toBeVisible();

await expect(
  page.getByText('+ QAR', { exact: false }).nth(1)
).toHaveText(`+ QAR ${expectedFBAmount}`);

// --- Gift Card Discount row (- QAR XX) ---
await expect(
  page.getByText('Gift Card Discount').nth(1)
).toBeVisible();

await expect(
  page.getByText('- QAR', { exact: false }).nth(1)
).toHaveText(`- QAR ${expectedDiscount}`);

// --- Total Price label ---
await expect(
  page.getByText('Total Price').nth(1)
).toBeVisible();
...
if (expectedTotalPrice === 0) {
  ...
  console.log('✅ Confirm Booking button is visible & enabled (Total Price = 0)');
...
test('TC_06 – Verify Direct F&B Gift Card Application and Removal for Item Without Modifiers', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
...
await applyAndRemoveGiftCardPayment(page, request,authToken, fbTracker.totalPrice);
...
test('TC_07 – Verify Direct F&B Order with Item Without Modifiers Using Gift Card and Credit Card Payment', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
...
test('TC_08 – Verify Direct F&B Order with Item Without Modifiers Using Novo Wallet Payment', async ({ page, request }) => {
  test.setTimeout(180000);
  page.setDefaultTimeout(120000);
...
await expect(
  page.getByText('Wallet Payment').nth(1)
).toBeVisible();
