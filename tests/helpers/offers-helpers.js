...
export async function verifyLoyaltyOffersAPI(
  page,
  cinemaId,
  sessionId,
  authToken,
  ...
  console.log("Parameters:", {
    cinemaId,
    cinemaIdType: typeof cinemaId,
    sessionId,
    sessionIdType: typeof sessionId,
    authToken: authToken?.substring(0, 30) + "...",
    userSessionId,
    userSessionIdLength: userSessionId?.length,
  });
  ...
  if (
    ...
    console.error(
      "❌ userSessionId appears to be a JWT token instead of a session hash!"
    );
    console.error(
      'Expected format: 32-character hash like "bf34fcf9d17f4d4b90fe750039071d9b"'
    );
  ...
  const numericCinemaId =
    typeof cinemaId === "number" ? cinemaId : parseInt(cinemaId, 10);
  const numericSessionId =
    typeof sessionId === "number" ? sessionId : parseInt(sessionId, 10);
  ...
  if (!authToken) {
    console.error("❌ Auth token missing for loyalty API");
    return null;
  }
  ...
  try {
    ...
    if (!loyaltyOffersResponse.ok()) {
      console.error(
        "Loyalty offers API failed:",
        loyaltyOffersResponse.status()
      );
    ...
    if (loyaltyMember?.LoyaltyMember) {
      ...
      console.log("Member:", {
        ...
        tierPoints: member.BalanceList?.find((b) => b.Name === "Tier Points")
          ?.PointsRemaining,
        spendPoints: member.BalanceList?.find((b) => b.Name === "Spend Points")
          ?.PointsRemaining,
      });
    }
    ...
    const processedOffers = tickets.map((ticket) => ({
      ...
      availableQuantity:
        ticket.QtyAvailable || ticket.QuantityAvailablePerOrder || 0,
...
export async function verifyBankOffers(page, request, sessionId, cinemaId) {
  await expect(page.getByRole("img", { name: "offerbg" }).first()).toBeVisible({
    timeout: 10000,
  });
  ...
  const activeOffers = offersData.data.filter(
    (offer) => offer.is_active && !offer.is_deleted
  );
  console.log(`Total active offers to verify: ${activeOffers.length}`);

  for (const offer of activeOffers) {
    const offerButton = page.getByRole("button", {
      name: new RegExp(offer.name, "i"),
    });
...
export function setupValidationInterceptors(page) {
  ...
  const offerValidatePromise = page.waitForResponse(
    (response) =>
      response.url().includes("/api/booking/offers/v2/validate") &&
      response.status() === 200,
    { timeout: 20000 }
  );
  ...
  await page.evaluate(
    ([validationData]) => {
      ...
      if (validationData.data?.verificationToken) {
        localStorage.setItem(
          "verification_token",
          validationData.data.verificationToken
        );
      }
      ...
      if (validationData.data?.applicableOffer) {
        ...
        localStorage.setItem(
          "offer_id",
          validationData.data.applicableOffer.id.toString()
        );
        localStorage.setItem(
          "offer_name",
          validationData.data.applicableOffer.offer_name
        );
      }
      ...
      if (validationData.data?.priceData) {
        localStorage.setItem(
          "discounted_price",
          validationData.data.priceData.discounted_price.toString()
        );
        localStorage.setItem(
          "final_price",
          validationData.data.priceData.final_price.toString()
        );
      }
    },
    [offerValidationData]
...
export function logOfferValidationResults(offerValidationData, panEntriesData) {
  if (
    ...
    console.warn(
      "⚠️ Offer validation returned isAllowed: false or isBinValid: false"
    );
    return false;
  }
}
...
export function setupPaymentInterceptors(page) {
  ...
  const offersApplyPromise = page
    .waitForResponse(
      (resp) =>
        resp.url().includes("/api/booking/offers/v2/apply") &&
        resp.request().method() === "POST",
      { timeout: 20000 }
    )
    .catch(() => null);
...
export async function captureBookingFeeAndDiscount(
  ...
  try {
    ...
    if (offersApplyResponse) {
      ...
      if (offersApplyData.success && offersApplyData.data) {
        offerApplied = true;
        bookingFeeCents =
          offersApplyData.data.updated_booking_fee ?? bookingFeeCents;
        actualDiscountCents =
          offersApplyData.data.discount_amount_in_cents ?? 0;
...
export async function captureBookingFeeAndLoyaltyDiscount(
  selectSeatsResponsePromise,
  loyaltyApplyPromise
) {
  let bookingFeeCents = 500;
...
export async function verifySeatsInPaymentPage(
  page,
  isOfferApplied,
  updatedSeatData,
  clickedSeats
  ...
  if (isOfferApplied && updatedSeatData) {
    ...
    for (const seatData of updatedSeatData) {
      ...
      try {
        await expect(page.getByText(seatIdentifier).first()).toBeVisible({
          timeout: 5000,
        });
      } catch {
...
export async function verifyTicketPricesInPayment(
  page,
  isOfferApplied,
  updatedSeatData
) {
  if (isOfferApplied && updatedSeatData) {
    ...
    for (const [key, ticketInfo] of ticketGroups) {
      ...
      try {
        ...
        console.log(
          `✓ Verified ticket price for ${ticketInfo.ticketName}: ${ticketPriceText}`
        );
      } catch {
        const priceRegex = new RegExp(
          `QAR\\s*${ticketInfo.unitPrice
            .toFixed(2)
            .replace(".", "\\.")}\\s*x\\s*${ticketInfo.count}`
        );
        try {
          ...
          console.log(
            `✓ Verified ticket price (regex) for ${ticketInfo.ticketName}: ${ticketPriceText}`
          );
        } catch {
...
export async function verifyPaymentDetails(
  ...
  try {
    ...
    try {
      await expect(page.getByText(ticketSubtotalExact).first()).toBeVisible({
        timeout: 2000,
      });
      ...
      try {
        await expect(
          page.getByText(ticketSubtotalNoDecimal).first()
        ...
        const ticketRegex = new RegExp(
          `\\+\\s*QAR\\s*${Math.floor(totalExpectedPrice)}(?:\\.00)?`
        );
        await expect(page.locator("body")).toContainText(ticketRegex, {
          timeout: 5000,
        });
        console.log(
          `✓ Verified ticket subtotal (regex): ${totalExpectedPrice}`
        );
      }
    }
  } catch {
  ...
  if (isOfferApplied && offerDiscountAmount > 0) {
    try {
      await expect(page.getByText("Bank Discount").first()).toBeVisible({
        timeout: 5000,
      });
      ...
      try {
        await expect(page.getByText(discountExact).first()).toBeVisible({
          timeout: 2000,
        });
        console.log(`✓ Verified bank discount: ${discountExact}`);
      } catch {
        try {
          await expect(page.getByText(discountNoDecimal).first()).toBeVisible({
            timeout: 2000,
          });
          ...
          const discountRegex = new RegExp(
            `-\\s*QAR\\s*${Math.floor(offerDiscountAmount)}(?:\\.00)?`
          );
          await expect(page.locator("body")).toContainText(discountRegex, {
            timeout: 5000,
          });
          console.log(
            `✓ Verified bank discount (regex): ${offerDiscountAmount}`
          );
        }
      }
    } catch (error) {
      console.warn(
        "Bank Discount section not found in side panel:",
        error.message
      );
    }
  }
  ...
  try {
    ...
    try {
      await expect(page.getByText(bookingFeeExact).first()).toBeVisible({
        timeout: 2000,
      });
      console.log(`✓ Verified booking fee: ${bookingFeeExact}`);
    } catch {
      try {
        await expect(page.getByText(bookingFeeNoDecimal).first()).toBeVisible({
          timeout: 2000,
        });
        ...
        const feeRegex = new RegExp(
          `\\+\\s*QAR\\s*${Math.floor(bookingFeeValue)}(?:\\.00)?`
        );
  ...
  try {
    ...
    const totalRegexExact = new RegExp(
      `Total Price.*QAR.*${totalExact.replace(".", "\\.")}`
    );
    try {
      await expect(page.getByText(totalRegexExact)).toBeVisible({
        timeout: 2000,
      });
      console.log(`✓ Verified total price: QAR ${totalExact}`);
    } catch {
      // Try non-decimal format
      const totalRegexNoDecimal = new RegExp(
        `Total Price.*QAR.*${totalNoDecimal}`
      );
      try {
        await expect(page.getByText(totalRegexNoDecimal)).toBeVisible({
          timeout: 2000,
        });
...
export async function verifyLoyaltyPaymentDetails(
  page,
  totalExpectedPrice,
  loyaltyOfferApplied,
...
export async function verifyOffersPromotionsSection(
  page,
  isOfferApplied,
  offerData
) {
  try {
      ...
      if (appliedOfferName) {
        try {
          ...
          if (appliedOfferName.toLowerCase().includes("cbq")) {
            await expect(page.getByText(/CBQ.*Offer/i).first()).toBeVisible({
              timeout: 5000,
            });
          }
        }
    ...
    console.warn(
      "Failed to verify Offers & Promotions section:",
      error.message
    );
  }
}

export async function verifyLoyaltyOffersSectionInPayment(
  page,
  loyaltyOfferApplied,
  selectedOffer
) {
...
export async function verifyFNBPageBasics(
  page,
  sidePanelApiData,
  movie,
  cinemaId,
