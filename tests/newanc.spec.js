  ...
  test("TC_07 – Verify “Premiere Club” Page Tiers, Membership Sections, Images, Terms Tabs, FAQs, Login State Behavior, and Language Switching Using Backend API Data", async ({
    ...
    if (en.bannerArray && en.bannerArray[0]?.banner) {
      const bannerImg = page
        .locator(".relative > .absolute.inset-0, img")
        .first();
    ...
    if (en.sectionTwoTitle) {
      await expect(page.locator("body")).toContainText(en.sectionTwoTitle, {
        timeout: 5000,
      });
    ...
    if (ar.sectionTwoTitle) {
      await expect(page.locator("body")).toContainText(ar.sectionTwoTitle, {
        timeout: 5000,
      });
      console.log(
